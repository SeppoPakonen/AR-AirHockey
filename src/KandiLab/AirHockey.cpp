...
void Map::Paint(WorldDraw& wdraw, Draw& draw) {
	...
	// Draw texts
	{
		GameTable* table = (GameTable*)this->table;
		String s;
		if (last_won != -1) {
			if (last_won == 0)
				s << "Player A won!";
...
void Player::MoveTo(const Pointf& target, double speed, double force_scalar) {
	...
	if (speed_diff_len < 1) {
		...
		double close_distance = 8;
		double factor = min(1.0, distance / close_distance);
		Pointf force = speed_diff / speed_diff_len * force_scalar * factor;
...
GameTable::GameTable() : pl_a(0), pl_b(1) {
	score_limit = 3;
	score[0] = 0;
	score[1] = 0;
	area_a.table = this;
...
void GameTable::PlayerScore(int i) {
	player_a_starts = i == 1;
	score[i]++;
...
void GameTable::ContactBegin(Contact contact) {
...
void GameTable::ContactEnd(Contact contact) {
	
}

void GameTable::ResetPuck() {
	if (puck.GetCount()) {
		Remove(puck[0]);
		puck.Remove(0);
	}
...
void GameTable::SetPlayer(Pointf p) {
